...
function main() {
  ...
  if (!prepared || flags.help) {
    console.log('Usage: node advance-sample-run.js --prepared runs/.../prepared-sample-run.json --apply-result apply.json [--config config.yaml --profile NAME --workspace WORKSPACE_ID --folder FOLDER --workbook WORKBOOK] [--verify verify.json --manifest manifest.json --counts rowsTested=10,errorCount=0] [--out-dir runs/.../] [--json]');
    return;
  }
