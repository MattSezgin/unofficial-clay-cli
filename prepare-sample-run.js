...
function main() {
  ...
  if (!requestText || flags.help) {
    console.log('Usage: node prepare-sample-run.js --request "Build a campaign personalization table" --inputs input.yaml [--config config.yaml --profile NAME --workspace WORKSPACE --folder FOLDER --workbook WORKBOOK] [--template template.yaml] [--out-dir runs/<date>/<id>] [--json]');
    return;
  }
