...
const PATTERN_DEFINITION_FILES = new Set([
  ...
  path.join('tests', 'test-public-readiness.js'),
  path.join('tests', 'test-redaction.js'),
  path.join('tests', 'fixtures', 'redaction-input.json'),
  path.join('tests', 'test-proof-packet.js'),
  path.join('tests', 'test-validate-config.js'), // fixture cookie 's%3A...ForTestsOnly' proves rejection works
...
const generatedChecks = [
  {
    ...
    args: [path.join(root, 'lib', 'plan-playbook.js'), path.join(root, 'playbooks', 'outbound-personalization.yaml'), '--inputs', path.join(root, 'examples', 'outbound-personalization-input.example.yaml'), '--json'],
  },
  {
    label: 'generated template plan',
    args: [path.join(root, 'lib', 'plan-playbook.js'), path.join(root, 'playbooks', 'outbound-personalization.yaml'), '--inputs', path.join(root, 'examples', 'outbound-personalization-input.example.yaml'), '--template-plan', 'outbound-personalization.yaml', '--json'],
  },
  {
    label: 'generated sample-run packet',
    args: [path.join(root, 'lib', 'plan-playbook.js'), path.join(root, 'playbooks', 'outbound-personalization.yaml'), '--inputs', path.join(root, 'examples', 'outbound-personalization-input.example.yaml'), '--sample-run', 'outbound-personalization.yaml', '--json'],
  },
];
