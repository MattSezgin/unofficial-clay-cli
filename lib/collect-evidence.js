...
function main() {
  ...
  if (flags.help) {
    console.log('Usage: node lib/collect-evidence.js [--apply apply.json] [--preflight preflight.json] [--hydrated-preflight preflight.json] [--verify verify.json] [--manifest manifest.json] [--counts rowsTested=10,errorCount=0] [--workspace ID --folder ID --workbook ID] [--out evidence.json]');
    return;
  }
