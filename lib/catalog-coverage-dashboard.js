...
const DEFAULT_RAW = path.join(REPO_ROOT, 'runs/2026-06-09/full-integration-audit-shards/actions-catalog.raw.json');
const DEFAULT_STRICT = path.join(REPO_ROOT, 'integration-library/registry.yaml');
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (arg === '--raw') args.raw = argv[++i];
      ...
      console.log('Usage: node lib/catalog-coverage-dashboard.js [--raw file] [--strict-registry file] [--json out.json] [--md out.md] [--limit 25]');
      process.exit(0);
    } else {
...
function publicSourcePath(file) {
  ...
  if (normalized.startsWith(REPO_ROOT.replace(/\\/g, '/'))) return path.relative(REPO_ROOT, file).replace(/\\/g, '/');
