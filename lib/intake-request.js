...
function buildIntake(requestText, opts = {}) {
  ...
  const scored = playbooks
    .map(item => ({ ...scorePlaybook(requestText, item.doc), file: path.relative(path.join(__dirname, '..'), item.file), doc: item.doc }))
  ...
  const inputFlag = opts.inputs ? ` --inputs ${path.relative(path.join(__dirname, '..'), opts.inputs)}` : ' --inputs <ignored-or-example-input.yaml>';
  ...
  return {
    ...
    nextCommands: {
      plan: `node lib/plan-playbook.js ${selected.file}${inputFlag} --json`,
      sampleRunPacket: `node lib/plan-playbook.js ${selected.file}${inputFlag} --sample-run ${templateSelector} --out runs/<date>/${selected.playbook.id}-sample-run.json`,
      preflight: `node lib/preflight-sample-run.js runs/<date>/${selected.playbook.id}-sample-run.json --config <config.yaml> --profile <profile> --workspace <workspace-id> --folder <sandbox-folder-id> --workbook <sandbox-workbook-id> --out runs/<date>/${selected.playbook.id}-preflight.json`,
    },
    stopConditions: [
...
function main() {
  ...
  if (!requestText || flags.help) {
    console.log('Usage: node lib/intake-request.js --request "Find people at these companies by job title" [--inputs input.yaml] [--out intake.json|yaml] [--json]');
    return;
  }
