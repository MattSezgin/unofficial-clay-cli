      ...
      commands: [
        {
          ...
          command: 'node lib/collect-evidence.js --apply <apply-result.json> --verify <verify.json> --manifest <redacted-manifest.json> --out <evidence.json>',
          mode: 'offline',
          confirmationRequired: false,
        },
        {
          commandId: 'write_quality_report',
          command: 'node lib/quality-report.js <plan.json> --evidence <evidence.json> --out <quality-report.md>',
          mode: 'offline',
          confirmationRequired: false,
        },
      ],
      ...
      commands: [
        {
          ...
          command: 'node lib/scale-gate.js --plan <plan.json> --evidence <evidence.json> --command "<exact Clay scale command with --confirm>" --quality-reviewed true --out <scale-gate.json>',
          mode: 'offline',
          confirmationRequired: false,
        },
      ],
...
function buildSampleRunPacketArtifact(plan, selector) {
  ...
  return {
    ...
    qualityReport: {
      command: 'node lib/quality-report.js <plan.json> --out runs/<date>/<playbook-id>-sample-quality-report.md',
...
function main() {
  ...
  if (!playbookFile || flags.help) {
    console.log('Usage: node lib/plan-playbook.js <playbook.yaml> [--inputs input.yaml] [--spec spec.yaml] [--specs a.yaml,b.yaml] [--template-plan spec.yaml|basename|index] [--sample-run spec.yaml|basename|index] [--out plan.yaml|json] [--json]');
    return;
  }
