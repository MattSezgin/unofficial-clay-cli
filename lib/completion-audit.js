...
function publicScanCovers(rel) {
  const scanner = readText(path.join('tests', 'test-public-readiness.js'));
...
function packageChecks() {
  const requiredFiles = [
    ...
    'examples',
    'specs/templates',
    'lib/intake-request.js',
    'lib/prompt-library.js',
    'lib/profile-context.js',
    'lib/prepare-sample-run.js',
    'lib/advance-sample-run.js',
    'lib/scale-gate.js',
    'lib/simulate-full-loop.js',
    'lib/completion-audit.js',
    'lib/workbook-parity.js',
  ];
...
function buildCompletionAudit(opts = {}) {
  ...
  const requirements = [
    requirement(
      ...
      exists('lib/intake-request.js') && scriptIncludes('test:intake') ? 'passed' : 'missing',
    ...
    requirement(
      ...
      pkg.exampleCount >= pkg.playbookCount && exists('lib/prepare-sample-run.js') ? 'passed' : 'missing',
    ...
    requirement(
      ...
      exists('lib/preflight-sample-run.js') && scriptIncludes('test:preflight') ? 'passed' : 'missing',
    ...
    requirement(
      ...
      exists('AGENTS.md') && exists('lib/scale-gate.js') && scriptIncludes('test:scale') ? 'passed' : 'missing',
...
function main() {
  ...
  if (flags.help) {
    console.log('Usage: node lib/completion-audit.js [--prepared file] [--apply-result file] [--advanced file] [--evidence file] [--quality-report file] [--scale-gate file] [--workbook-parity file] [--simulation file] [--json]');
    return;
  }
